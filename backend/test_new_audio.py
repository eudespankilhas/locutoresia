...
# Testar o novo arquivo gerado
def test_new_audio():
    print("Testando novo arquivo gerado com síntese de fala...")
            ...
            if n_frames > 0:
                ...
                if len(frames) > 0 and sample_width == 2:
                    ...
                    if max_val == 0:
                        ...
                        print("SUCESSO: Áudio contém dados de fala sintetizada!")
                        
                        # Mostrar variação para indicar fala em vez de tom constante
                        print(f"Variação dos valores: min={min(values[:100])}, max={max(values[:100])}")
                        
                        # Verificar se há variação suficiente para ser fala
                        variation = max(values[:100]) - min(values[:100])
                        if variation > 1000:
                            print("INDICADOR: Variação alta sugere síntese de fala!")
                        else:
...
if __name__ == "__main__":
    success = test_new_audio()
    if success:
        print("\n=== RESULTADO ===")
        print("Síntese de fala está funcionando!")
        print("O áudio agora deve corresponder melhor ao texto digitado.")
    else:
        print("\n=== RESULTADO ===")
        print("Ainda há problemas na síntese de fala.")

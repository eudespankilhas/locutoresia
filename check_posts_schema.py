...
# Tenta buscar colunas da tabela posts
try:
    response = requests.get(
        f"{supabase_url}/rest/v1/posts?select=*&limit=1",
        headers=headers,
        timeout=10
    )
    ...
    if response.status_code == 200:
        ...
        if data and len(data) > 0:
            print("Colunas encontradas na tabela posts:")
            for col in data[0].keys():
                print(f"  - {col}")
        else:
            print("Tabela posts existe mas está vazia")
    else:
...
print("\nVerificando se tabela posts existe...")
try:
    response = requests.get(

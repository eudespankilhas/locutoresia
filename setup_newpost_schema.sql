...
DO $$
BEGIN
    -- Adicionar author_id (UUID, nullable para compatibilidade)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'posts' AND column_name = 'author_id') THEN
        ALTER TABLE posts ADD COLUMN author_id UUID REFERENCES newpost_profiles(id);
        COMMENT ON COLUMN posts.author_id IS 'ID do autor do post (referencia newpost_profiles)';
    END IF;
...
-- 6. Atualizar registros existentes do Locutores IA para ter author_id
-- Usando o autor padrao (usuario principal)
UPDATE posts 
